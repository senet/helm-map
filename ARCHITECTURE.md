...
> **Note:** Only `platformCommand` is used (no top-level `command` field). Earlier Helm v3
> versions warned when both fields were present; this has been removed for forward compatibility.

**Install script (`scripts/install.sh`):**

The install script first tries to download a pre-built binary from GitHub Releases. If no release
is available (HTTP 404), it falls back to building from source using the Go toolchain.
...
URL="https://github.com/senet/helm-map/releases/download/v${PLUGIN_VERSION}/helm-map_${OS}_${ARCH}.tar.gz"
mkdir -p "$HELM_PLUGIN_DIR/bin"

# Try download; fall back to source build
if ! curl -sSL -f "$URL" | tar -xz -C "$HELM_PLUGIN_DIR/bin" 2>/dev/null; then
  echo "No pre-built release found, building from source..."
  cd "$HELM_PLUGIN_DIR"
  go build -o "$HELM_PLUGIN_DIR/bin/helm-map" ./cmd/helm-map/
fi
...
# Helm v4 (until signed releases are published)
...
- GPG-signed release archives — each `.tar.gz` has a corresponding `.prov` detached signature generated by goreleaser
- Public key published in the repository (`helm-map.pub`) for manual verification
